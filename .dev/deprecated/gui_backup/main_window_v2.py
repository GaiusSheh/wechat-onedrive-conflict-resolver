...
class MainWindow:
    ...
    def show_config_dialog(self):
        ...
        # 静置触发配置
        idle_frame = ttk.LabelFrame(main_frame, text="静置触发设置", padding="15")
        idle_frame.grid(row=0, column=0, sticky="ew", pady=(0, 15))
        idle_frame.columnconfigure(2, weight=1)
        
        idle_enabled_var = tk.BooleanVar(value=self.config.is_idle_trigger_enabled())
        ...
        idle_check.grid(row=0, column=0, sticky=tk.W, pady=5)
        
        ttk.Label(idle_frame, text="静置时间(分钟):").grid(row=1, column=0, sticky=tk.W, pady=5)
        
        idle_minutes_var = tk.StringVar(value=str(self.config.get_idle_minutes()))
        idle_spinbox = ttk.Spinbox(
            idle_frame, 
            from_=1, 
            to=120, 
            width=10, 
            textvariable=idle_minutes_var,
            bootstyle="info"
        )
        idle_spinbox.grid(row=1, column=1, sticky=tk.W, padx=(10, 0), pady=5)
        
        # 定时触发配置
        scheduled_frame = ttk.LabelFrame(main_frame, text="定时触发设置", padding="15")
        scheduled_frame.grid(row=1, column=0, sticky="ew", pady=(0, 15))
        scheduled_frame.columnconfigure(2, weight=1)
        
        scheduled_enabled_var = tk.BooleanVar(value=self.config.is_scheduled_trigger_enabled())
        scheduled_check = ttk.Checkbutton(
            scheduled_frame, 
            text="启用定时触发", 
            variable=scheduled_enabled_var,
            bootstyle="round-toggle"
        )
        scheduled_check.grid(row=0, column=0, sticky=tk.W, pady=5)
        
        ttk.Label(scheduled_frame, text="触发时间:").grid(row=1, column=0, sticky=tk.W, pady=5)
        
        scheduled_time_var = tk.StringVar(value=self.config.get_scheduled_time())
        time_entry = ttk.Entry(
            scheduled_frame, 
            width=15, 
            textvariable=scheduled_time_var,
            bootstyle="info"
        )
        time_entry.grid(row=1, column=1, sticky=tk.W, padx=(10, 0), pady=5)
        ttk.Label(scheduled_frame, text="(格式: HH:MM)").grid(row=1, column=2, sticky=tk.W, padx=(10, 0), pady=5)
